...
predictions_class <- ifelse(knn_results$pred$Class1 > 0.9, "Class1", "Class0")
...
conf_matrix1 <- confusionMatrix(xtab, positive = "Class1")
...
print(FPR6)
```
We see that even with threshold optimization, our confusion matrix produces 0 for TPR and FPR of .028. This means it identified none of the positive cases correctly. It also was able to identify most negative cases as negative. However, since we have not been able to distinguish between the two classes well with knn based on the confusion matrix, we will say it is not effective for our purposes.
