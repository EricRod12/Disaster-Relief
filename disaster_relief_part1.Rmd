...
Next we will do the KNN model. We will explore a range of K values between 2 and 20 and try to assess the best value for k based on metrics and performance with each k value within our 10 folds. We are also going to normalize the data prior to fitting the model.
...
cmknntuned_test <- calculate_conf_mat_test(tuned_nn_wf, test, threshold = max_j_index_threshold_knn)
