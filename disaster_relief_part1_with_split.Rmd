...
The analysis was conducted using R, which is an environment for statistical computing and graphics. The tidymodels framework utilized for modeling and validation processes uses several packages including dplyr for data manipulation, rsample for data splitting, yardstick for model evaluation, and caret and glmnet for modeling. Additionally, the MASS and discrim and pROC packages were employed for discriminant analysis, threshold optimization, and ROC curve analysis, respectively. The patchwork package facilitated the arrangement of multiple ggplot objects for comparative visualization.

Parameter Tuning and Model Selection

Model selection was informed by cross-validtaion and performance metrics evaluation. A 10 fold cross validation strategy was used to ensure robust model validation and to mitigate the risk of overfitting, This approach divides the data into ten parts, with each part used once as a validation set while the rest serve as a training set, thereby allowing the model to be trained and validated on different subsets of the data. We used parameter tuning for our penalized logistic regression model and knn model by testing alpha and lambda values in penalized logistic regression and by testing the highest performing of k in the KNN model. In Logistic Regression, LDA and QDA, the primary focus was on threshold optimization for each fold.
...
```
Below are the performance metrics for each of the three models: Logistic Regression, QDA, and LDA on the test set with the mean of the optimal thresholds of each fold as the chosen value for threshold. We chose this because all of our folds perform well, so the mean of the threshold for each fold makes sense for to evaluate the test set. In addition, there is an increase in performance from the models without threshold optimization. 
...
```
QDA performs the best among these three, since all of the performance metrics are the highest in the above table. Below are the confusion matrices for all three models on the test set with the optimal thresholds.
